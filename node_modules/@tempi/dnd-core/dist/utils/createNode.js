...
var NodeContext = require('../nodes/NodeContext.js');
var Element = require('../nodes/Element.js');
...
function createNode(newNode, normalize) {
  ...
  const id = newNode.id || dndUtils.getRandomId();
  const node = {
    id,
  ...
  if (node.data.type === Element.Element || node.data.type === Canvas.Canvas) {
    const mergedProps = __spreadValues(__spreadValues({}, Element.defaultElementProps), node.data.props);
    node.data.props = Object.keys(node.data.props).reduce((props, key) => {
      if (Object.keys(Element.defaultElementProps).includes(key)) {
        node.data[Element.elementPropToNodeData[key] || key] = mergedProps[key];
      } else {
    ...
    const usingDeprecatedCanvas = node.data.type === Canvas.Canvas;
    if (usingDeprecatedCanvas) {
      node.data.isCanvas = true;
      Canvas.deprecateCanvasComponent();
    }
  }
  if (normalize) {
    normalize(node);
  ...
  if (userComponentConfig) {
    ...
    if (userComponentConfig.related) {
      ...
      Object.keys(userComponentConfig.related).forEach((comp) => {
        node.related[comp] = () => React__default["default"].createElement(
          NodeContext.NodeProvider,
          relatedNodeContext,
          React__default["default"].createElement(userComponentConfig.related[comp])
        );
      });
    }
  }
