...
var findPosition = require('./findPosition.js');
...
const _Positioner = class {
  ...
  onScroll(e) {
    ...
    const rootNode = this.store.query.node(dndUtils.ROOT_NODE).get();
  ...
  getDraggedNodes() {
    if (this.dragTarget.type === "new") {
      return getNodesFromSelector.getNodesFromSelector(
  ...
  computeIndicator(dropTargetId, x, y) {
    ...
    if (newParentNode.data.parent && this.isNearBorders(dndUtils.getDOMInfo(newParentNode.dom), x, y) && !this.store.query.node(newParentNode.id).isLinkedNode()) {
