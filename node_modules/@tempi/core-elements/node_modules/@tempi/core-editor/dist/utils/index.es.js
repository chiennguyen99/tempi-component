...
import { GRID_COLUMNS } from '../constants/index.es.js';
...
const serializePB = (query, options) => {
  ...
  Object.keys(result).forEach((key) => {
    ...
    if (!canSerialize) {
      ...
      let children = [];
      if (tag === "galleryslideshow" && obj.parent === ROOT) {
        tag = "slider";
      }
...
const deserializePB = (tree, isEditingBlock, isEditingPopup) => {
  if (!tree || Object.keys(tree).length === 0)
    return "";
  const result = Object.assign({}, cloneDeep(tree));
  const rootBlockId = [];
  Object.keys(result).forEach((key) => {
    var _a, _b;
    const obj = result[key];
...
const convertGridToPercentage = (customAttributes) => {
  ...
  if (typeof md === "number") {
    customAttributes.md = {
      flex: percentage(md, GRID_COLUMNS)
    };
  }
  if (typeof xs === "number") {
    customAttributes.xs = {
      flex: percentage(xs, GRID_COLUMNS)
    };
  }
  ...
  for (const key in pageContent) {
    attributes.forEach((element) => {
      if (pageContent[key].tag === element.tag) {
        set(pageContent[key], element.path, value);
      }
    });
  }
};
const clonePageContent = (pageContent, callbackTransformNestedContent, returnMappingKeys = false) => {
  ...
  const level1Keys = Object.keys(pageContent).filter(
    (el) => pageContent[el].parentId === ROOT
  );
  const anchor = parseInt(generateRandomId(10).replaceAll(".", ""), 36);
  ...
  for (const k in pageContent) {
    if (k === ROOT)
      ...
      mappingOldNewKey[k] = generateRandomId(10);
  }
...
export { clonePageContent, deserializePB, floorPercentage, percentage, resolvedNameMapping, serializePB, setPbConfigProperties };
