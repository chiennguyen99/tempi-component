...
function useEditor(collect) {
  const _a = dndCore.useEditor(collect), { actions, query } = _a, collected = __objRest(_a, ["actions", "query"]);
  const resizeColumns = (rowId, nodesLength) => {
    ...
    childNodes.forEach(
      (el) => actions.history.throttle().setProp(el, (props) => {
        set__default["default"](props, "md.flex", `${index.floorPercentage(100 / nodesLength)}%`);
      })
    );
  };
  const generateId = (node) => {
    ...
    try {
      ...
      let newNodeId = coreRenderer.generateRandomId(10);
      ...
      return coreRenderer.generateRandomId(10);
    }
  };
  ...
  const addBlock = async (serializeNodes, index, parentId = coreRenderer.ROOT, rootNodeId) => {
    ...
    if (!rootNodeId) {
      const parent = nodes.find((el) => el.data.parent === coreRenderer.ROOT);
  ...
  const onCopy = async (nodeId) => {
    try {
      const pageContent = index.serializePB(JSON.parse(query.serialize()), {
      ...
      antd.message.success({ content: i18n.t("Sao ch\xE9p th\xE0nh c\xF4ng") });
    } catch (err) {
      antd.message.error({ content: `${err.name}: ${err.message}.` });
    }
  };
  const onPaste = async (index$1, parentId, callbackTransformNestedContent) => {
    try {
      ...
      const [copyContent, mappingOldNewKey] = index.clonePageContent(
        nodeContent,
        callbackTransformNestedContent,
        true
      );
      const insertNodes = index.deserializePB(copyContent);
      addBlock(
        insertNodes,
        index$1,
        parentId,
  ...
  const getLastSection = (parentId = coreRenderer.ROOT) => {
