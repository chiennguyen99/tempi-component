...
var util = require('./colors/util.js');
...
const getFourSideCss = (props) => {
  ...
  return Object.entries(props).reduce((acc, curr) => {
    const [key, value = {}] = curr;
    const _a = value, { unit = style.UnitEnum.pixel } = _a, restValue = __objRest(_a, ["unit"]);
...
const getBackgroundOverlay = (type, value) => {
  if (type === style.OverlayType.color && value) {
    return `
      & {
        position: relative;
...
const getBackgroundImageUrl = (bgImage, bgImageWidth, bgImagePercent) => {
  ...
  if (!isUrlPrefix && bgImage) {
    return `url(${image.getImageUrlFromGoogleUserContent(bgImage, {
      width: bgImageWidth,
  ...
  if (!(backgroundVideo == null ? void 0 : backgroundVideo.src) && !(backgroundVideo == null ? void 0 : backgroundVideo.uploadSrc)) {
    ...
    if (backgroundImage) {
      ...
      bgCss += bgImage !== bgImagePlaceHolder && lazyBgImage ? `
        ...
        &.${common.BG_VISIBLE_LAZY_CLASS_NAME} {
...
const unitParse = (value, unit = style.UnitEnum.pixel) => {
  if (typeof value === "number")
    return `${value}${unit}`;
  return value;
};
...
const stringifyColor = (color) => {
  if (color.colorType === style.ColorType.solid)
  ...
  const gradientRadius = gradientColor.gradientType === style.GradientType.radial ? "circle" : `${gradientColor.radius}deg`;
...
const parseColor = (color) => {
  ...
  if (!color.includes("-gradient"))
    return {
      colorType: style.ColorType.solid,
      colorValue: color.includes("#") ? util.getRgbaFromHex(color) : color
    };
  ...
  return {
    colorType: style.ColorType.gradient,
    colorValue: {
      colors: [...colorList],
      radius: parseInt(radius) || void 0,
      gradientType: gradientType === style.GradientType.linear ? style.GradientType.linear : style.GradientType.radial
    }
  };
};
...
exports.getBackgroundImageMarquee = getBackgroundImageMarquee;
