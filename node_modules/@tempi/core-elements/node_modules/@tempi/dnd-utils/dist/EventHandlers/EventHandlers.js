...
var ConnectorRegistry = require('./ConnectorRegistry.js');
var interfaces = require('./interfaces.js');
var isEventBlockedByDescendant = require('./isEventBlockedByDescendant.js');
...
class EventHandlers {
  constructor(options) {
    this.registry = new ConnectorRegistry.ConnectorRegistry();
  ...
  disable() {
    ...
    this.subscribers.forEach((listener) => {
      listener(interfaces.EventHandlerUpdates.HandlerDisabled);
    });
  }
  enable() {
    if (this.onEnable) {
    ...
    this.subscribers.forEach((listener) => {
      listener(interfaces.EventHandlerUpdates.HandlerEnabled);
    });
  }
  cleanup() {
    this.disable();
  ...
  addCraftEventListener(el, eventName, listener, options) {
    const bindedListener = (e) => {
      if (!isEventBlockedByDescendant.isEventBlockedByDescendant(e, eventName, el)) {
