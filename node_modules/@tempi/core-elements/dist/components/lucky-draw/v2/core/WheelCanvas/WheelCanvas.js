...
const drawWheel = async (canvasRef, items, config) => {
  ...
  if (canvas == null ? void 0 : canvas.getContext("2d")) {
    ...
    const clampedTextDistance = utils.clamp(0, 100, textDistance);
    const textRadius = outsideRadius * clampedTextDistance / 100;
    const clampedInsideRadius = utils.clamp(0, 100, innerRadius);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const { style } = items[i];
      ctx.fillStyle = (style == null ? void 0 : style.backgroundColor) || constants.LUCKY_DRAW_DEFAULT_COLOR;
      ...
      if (imgSrc) {
        ...
        img.src = imgSrc;
        img.onload = function() {
          const { width, height } = utils.calculateImageSize(
            img.width,
            img.height,
            imgWidth,
...
const WheelCanvas = (props) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(styles.WheelCanvasStyle, {
    ref: canvasRef,
    width,
    height
  });
