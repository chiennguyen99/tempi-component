...
import { useStore, Api, publicBffUrl } from '@tempi/core-renderer';
import get from 'lodash.get';
import { useState, useEffect } from 'react';
import { ERROR_CODES } from './constants.es.js';
...
const useLuckyDraw = (id, configs) => {
  ...
  const [listContentHtml, setListContentHtml] = useState();
  ...
  const API = new Api(`${publicBffUrl[env]}/v1/lucky-draw`);
  const handleSpin = async (userInfo2) => {
    ...
    await API.post("/spin", { wheelId: id, userInfo: userInfo2 }).then((res) => {
      ...
      var _a3, _b2;
      if (parseInt((_a3 = error == null ? void 0 : error.response) == null ? void 0 : _a3.code) === ERROR_CODES.OUT_OF_TURN) {
        showPopup(popupResponse == null ? void 0 : popupResponse.outOfTurn);
        setTurnCount(0);
        return;
      }
      if (parseInt((_b2 = error == null ? void 0 : error.response) == null ? void 0 : _b2.code) === ERROR_CODES.OUT_OF_PRIZE) {
  ...
  const replaceRegex = (popupId, prize, str) => {
    ...
    popupEle.querySelectorAll(".popup-content p, h1, h2, h3, h4, h5, h6").forEach((el, index) => {
      el.innerHTML = str[index].replaceAll(searchRegExp, (i) => {
        return get(prize, i.replace(/{{/, "").replace(/}}/, ""));
      });
    });
  };
  ...
  const handleCustomResponse = (apiResponse) => {
    ...
    popupResponse.custom.forEach((item) => {
      var _a3, _b, _c, _d;
      if (((_b = (_a3 = get(apiResponse, item.key)) == null ? void 0 : _a3.toString) == null ? void 0 : _b.call(_a3)) === ((_c = item.value) == null ? void 0 : _c.toString())) {
