...
import { Api, useStore, publicBffUrl } from '@tempi/core-renderer';
import { useState, useRef } from 'react';
import { ContentType } from './type.es.js';
import { formatData } from './utils.es.js';
...
import { isUrlValid } from '../../../utils/isUrlValid.es.js';
...
const useForm = ({
  ...
  const { env } = useStore();
  const popupContentTemplates = useRef({});
  const handleCallAPI = async (submit, oldValues, inputs) => {
    ...
    if (config == null ? void 0 : config.autoPassParams) {
      newValues = __spreadValues(__spreadValues({}, queryStringToObject(window.location.search)), newValues);
    }
    if (submitType === "api") {
      ...
      if (["PATCH", "POST"].includes(apiConfig == null ? void 0 : apiConfig.method)) {
        const contentType = headers["Content-Type"] || (apiConfig == null ? void 0 : apiConfig.contentType) || ContentType.urlEncoded;
        return api.post(apiConfig == null ? void 0 : apiConfig.url, formatData(contentType, values), {
    ...
    if (submitType === "googleSheet") {
      const values = transformValuesGGSheet(newValues, inputs);
      const ggSheetConfig = config;
      if (!ggSheetConfig.valid || ggSheetConfig.formAccountId <= 0) {
        const err = t("M\u1ED9t k\u1EBFt n\u1ED1i Google Sheet ch\u01B0a s\u1EB5n s\xE0ng, ki\u1EC3m tra l\u1EA1i!");
        console.error(err);
        return Promise.reject(err);
      }
      const date = new Date();
      ...
      return formUtils.insertDataToGoogleSheetV2(
        ggSheetConfig,
  ...
  const handleOpenPopup = (popupId, apiResponse, templateContent) => {
    ...
    popupContent.querySelectorAll("p, h1, h2, h3, h4, h5, h6").forEach((el, index) => {
      ...
      el.innerHTML = templateContent[index].replaceAll(searchRegExp, (i) => {
        return get(data, i.replace(/{{/, "").replace(/}}/, ""));
      });
    });
    popupEle == null ? void 0 : popupEle.click();
  };
  const handleSuccess = (apiResponse) => {
    if (!!(response == null ? void 0 : response.successPage) && isUrlValid(response.successPage))
  ...
  const handleFailed = (apiResponse) => {
    if (!!(response == null ? void 0 : response.failedPage) && isUrlValid(response.failedPage))
  ...
  const handleCustomResponse = (apiResponse) => {
    ...
    response.custom.forEach((item) => {
      var _a2, _b, _c;
      if (((_b = (_a2 = get(apiResponse == null ? void 0 : apiResponse.data, item.key)) == null ? void 0 : _a2.toString) == null ? void 0 : _b.call(_a2)) === ((_c = item.value) == null ? void 0 : _c.toString())) {
        handleOpenPopup(
          item.popup,
          apiResponse,
  ...
  const handleSubmitFormData = async (configStorage, values, inputs) => {
    ...
    try {
      ...
      const { year, month, day, hour, minute, second } = getPartialDateTimeTimeZone7(date);
      const dateString = `${year}-${month}-${day} ${hour}:${minute}:${second}`;
      let valuesSubmit = transformValuesGGSheet(newValues, inputs);
      valuesSubmit = [
        ...valuesSubmit,
        {
          name: "Sending time",
  ...
  const transformValues = (values, inputs) => {
    ...
    return Object.entries(values).map(([k, v]) => {
      if (dateFields.includes(k) && !!v) {
        const { year, month, day } = getPartialDateTimeTimeZone7(
          new Date(v)
        );
        return [k, `${day}-${month}-${year}`];
      }
      if (timeFields.includes(k) && !!v) {
        const { hour, minute, second } = getPartialDateTimeTimeZone7(
          new Date(v)
        );
        return [k, `${hour}:${minute}:${second}`];
      }
      if (dateTimeFields.includes(k) && !!v) {
        const { year, month, day, hour, minute, second } = getPartialDateTimeTimeZone7(new Date(v));
