import { getPartialDateTimeTimeZone7 } from './getPartialDateTimeTimeZone7.es.js';
...
const transformValuesGGSheet = (values, inputs) => {
  ...
  inputs.forEach((item) => {
    ...
    if (item.type === "datetime-local" && !!values[item.name]) {
      const { year, month, day, hour, minute, second } = getPartialDateTimeTimeZone7(new Date(values[item.name]));
