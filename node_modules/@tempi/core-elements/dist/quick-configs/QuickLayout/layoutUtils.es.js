...
const getLayoutStringByColumnNumber = (n) => {
  if (n < 1)
    return "";
  const number = floorPercentage(100 / n);
  let line = "";
  for (let i = 1; i <= n; i++) {
    line += `${number}%`;
    if (i < n) {
  ...
  if (value) {
    ...
    if (!pattern.test(value)) {
      return Promise.reject(
        t("Vui l\xF2ng nh\u1EADp \u0111\xFAng c\xFA ph\xE1p (v\xED d\u1EE5: 50% + 50% )")
      );
    }
...
export { dissectStringIntoPercentages, getLayoutStringByColumnNumber, layoutIcons, validateLayoutSyntax };
