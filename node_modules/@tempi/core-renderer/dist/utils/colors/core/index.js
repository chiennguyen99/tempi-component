...
var index = require('./parse-css-color/index.js');
var index$1 = require('./mix-css-color/index.js');
var index$2 = require('./pure-color/index.js');
...
class ColorCore {
  ...
  tint(weight, w = defaultNumberParam(weight, 50)) {
    return new ColorCore(
      `rgb(${index$1("#fff", this.rgbString(), w).rgba})`,
      "tint",
      w
    );
  }
  shade(weight, w = defaultNumberParam(weight, 50)) {
    return new ColorCore(
      `rgb(${index$1("#000", this.rgbString(), w).rgba})`,
      "shade",
      w
    );
  }
  ...
  hexString() {
    return index$2.rgb2hex(this.alpha >= 1 ? this.rgb : [...this.rgb, this.alpha]);
  }
  rgbString() {
  ...
  _setFromHSL([h, s, l, a]) {
    [this.rgb, this.alpha] = [index$2.hsl2rgb([h, s, l]).map(Math.round), a];
    return this;
  }
}
